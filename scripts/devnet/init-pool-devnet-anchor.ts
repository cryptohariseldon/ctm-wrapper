...
async function initializePoolWithClient() {
  console.log('Initializing CP-Swap pool on Devnet using Anchor...\n');
  ...
  // Load the program with devnet program ID
  ...
  // Override program ID for devnet
  idl.address = CP_SWAP_PROGRAM_ID.toBase58();
  ...
  const initAmount0 = new BN(1000 * Math.pow(10, token0Decimals)); // 1000 token0
  const initAmount1 = new BN(1 * Math.pow(10, token1Decimals)); // 1 token1
  ...
  // Fetch AMM config to get the protocol owner
  const ammConfigData = await program.account.ammConfig.fetch(ammConfig);
  console.log('\nAMM Config details:');
  console.log('- protocolOwner:', ammConfigData.protocolOwner.toBase58());
  console.log('- fundOwner:', ammConfigData.fundOwner.toBase58());
  console.log('- createPoolFee:', ammConfigData.createPoolFee.toString());
  
  // The fee account should be the protocol owner's token account
  const protocolOwnerToken0 = getAssociatedTokenAddressSync(token0Mint, ammConfigData.protocolOwner);
  console.log('- Expected fee account:', protocolOwnerToken0.toBase58());
  
  // Use the protocol owner's token account
  const createPoolFeeAccount = protocolOwnerToken0;
